...
3. If scopes are incorrect, generate a new PAT, update the credential in the [Warp Vault](../../warp-vault/index) app, and push the updated vault file to Migration HQ.
...
3. Update the credential in the [Warp Vault](../../warp-vault/index) app and push the updated vault file to Migration HQ.
...
1. Open the [Warp Vault](../../warp-vault/index) app and verify your credentials are current.
...
1. Use the [`/rename-destination`](../../slash-commands/migration/rename-destination) slash command to change the destination repository name before retrying the migration.
...
Runner Agent logs provide detailed error information that can help identify the root cause. See [Troubleshooting Using a Runner Agent's Logs](../../migration-hq/runner-agent) for instructions on locating and reading log output.

## Related Pages

* [Failed Inventory Scan](failed-inventory-scan) — similar causes apply to scan failures
* [Main Branch Requirement](main-branch-requirement) — runner failures caused by branch naming
* [Troubleshooting](index) — index of all common issues
* [Support](../index) — contact Product Support
